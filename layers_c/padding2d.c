...
uint8_t padding2d_fix16(uint16_t padding_height, uint16_t padding_width,
	...
	uint16_t o_slice_length = (2 * padding_height + input_height) * (2 * padding_width + input_width);
	uint16_t i_slice_length = input_height * input_width;
	uint16_t o_count = 0, i_count = 0;

	for(uint16_t depth = 0; depth < input_depth; depth++){
		for(uint16_t i = 0; i < padding_height * (padding_height * 2 + input_width); i++){
			output[o_count] = 0;
			o_count += 1;
		}
