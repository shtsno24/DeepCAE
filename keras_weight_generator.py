...
def write_weight_SeparableConv2D_c(weight_depthwise, weight_pointwise, bias, file_name, weight_array_name, bias_array_name, type_name, isFixed=False, fractal_width=0, array_type=None):
    ...
    with open(file_name, 'w') as f:
        ...
        f.write("{%d, %d, %d, %d};\n" % (weight_depthwise.shape[0], weight.shape[1], weight.shape[2], weight.shape[3]))
        ...
        for layers in model_layers_itr["config"]["layers"]:
            ...
            if(layers["class_name"].find("UpSampling") == -1 and layers["class_name"].find("MaxPooling2D") == -1 and layers["class_name"].find("UpSampling2D") == -1):
                ...
                write_weight_Conv2D_cpp(param_w,
                                        param_b,
                                        "weights_cpp/" + layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_fix16.h",
                                        layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_w",
                                        layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_b",
                                        "int16_t", isFixed=True, fractal_width=fractal, array_type=np.int16)
                ...
                write_weight_Conv2D_cpp(param_w,
                                        param_b,
                                        "weights_cpp/" + layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_float32.h",
                                        layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_w",
                                        layers["class_name"] + "_" + str(int(itr_counter / 2) - 1) + "_b",
                                        "float")
